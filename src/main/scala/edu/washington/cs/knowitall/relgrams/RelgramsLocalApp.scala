...
object RelgramsLocalApp {
  ...
  def main(args:Array[String]){
    ...
    val parser = new OptionParser() {
      ...
      opt("skipSentences", "Skip sentences as part of relgram tuples.", {str => skipSentences = str.toBoolean})
    ...
    extractor = new RelgramsExtractor(maxWindow, equality, noequality, multipleEquality, skipSentences)
    counter = new RelgramsCounter(maxSize)
    val tupleDocuments = loadTupleDocuments(inputPath)

    
    val extracts = extractRelgramCountsAndTuples(tupleDocuments, maxSentenceLength, equality, noequality)

    val reducedRelgramCounts = reduceRelgramCounts(extracts.map(x => x._1), minDirFreq)
  ...
  def extractRelgramCountsAndTuples(tuplesDocuments: Seq[TuplesDocumentWithCorefMentions], maxSentenceLength:Int, equality:Boolean, noequality:Boolean): Seq[(Map[String, RelgramCounts], Map[String, RelationTupleCounts])] ={
    tuplesDocuments.flatMap(document => {
      ...
      try{
        Some(extractor.extractRelgramsFromDocument(document, maxSentenceLength))
      }catch{
        case e:Exception => {
  ...
  def reduceRelgramCounts(relgramCounts: Iterable[Map[String, RelgramCounts]], minDirFreq:Int) = {
    relgramCounts.flatten
                 .groupBy(x => x._1)
                 .flatMap(x => counter.reduceRelgramCounts(x._2.map(y => y._2), minDirFreq))
  }
