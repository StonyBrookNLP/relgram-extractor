...
object StringUtils {
  ...
  def toIntOrElse(string:String, orElse:Int) = try { string.toInt } catch {case e:Exception => orElse}

  def toDoubleOrElse(string:String, orElse:Double) = try { string.toDouble } catch {case e:Exception => orElse}
