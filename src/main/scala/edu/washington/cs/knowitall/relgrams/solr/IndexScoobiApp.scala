...
object IndexScoobiApp extends ScoobiApp {
  ...
  def run() {
    ...
    val parser = new OptionParser() {
      ...
      opt("verbOnly", "verbonly.", {str => verbOnly = str.toBoolean})
    ...
    def isVerbOnly(measures:Measures): Boolean = {
      def isVerb(tuple:RelationTuple) = tuple.rel.split(" ").size <= 1
      val relgram = measures.urgc.rgc.relgram
      isVerb(relgram.first) && isVerb(relgram.second)
    }
    ...
    val output:DList[Measures]  = TextInput.fromTextFile(inputPath).flatMap(line => Measures.fromSerializedString(line) match {
      case Some(measures:Measures) => {
        if (aboveThreshold(measures) && (!verbOnly || isVerbOnly(measures))){
